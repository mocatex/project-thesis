...
The playground is run-centric: the user selects a run_id corresponding to a previously executed pipeline run. \\
The implementation provides a small run management module that lists recent runs and fetches detailed run information from the pipeline_runs table, including fields such as question, final_answer, and additional stored debug artifacts. These run-level fields serve two purposes:
...
In addition to run metadata, the playground queries the per-run working dataset stored in pipeline_run_articles (linked to the main corpus table article_corpus). The design intentionally queries only a reduced, run-specific slice rather than the entire corpus to keep visualization computation bounded and directly relevant to the specific analytical question.
